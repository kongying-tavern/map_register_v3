...
const rules: ItemFormRules<API.NoticeVo & { validDateTime: [number, number] }> = {
  ...
  validTimeStart: [
    {
      required: !!(formData.value?.validTimeStart && formData.value?.validTimeEnd),
      validator (rule, value, cb) {
        if (value && formData.value.validTimeEnd) {
          if (formData.value.validTimeEnd <= value) {
            cb("失效时间不能大于发布时间")
            return
          }
        }
  ...
  validTimeEnd: [
    {
      required: !!(formData.value?.validTimeStart && formData.value?.validTimeEnd),
      validator (rule, value, cb) {
        if (value && formData.value.validTimeStart) {
          if (formData.value.validTimeStart >= value) {
            cb("失效时间不能大于发布时间")
            return
          }
        }
...
const colors = [
  ...
  "#FF0000", "#FFFF00", "#80FF00", "#00FF40", "#00FFFF", "#0080C0", "#0080C0", "#FF00FF",
  "#804040", "#FF8040", "#00FF00", "#008080", "#004080", "#8080FF", "#800040", "#FF0080",
  "#800000", "#FF8000", "#008000", "#008040", "#0000FF", "#0000A0", "#800080", "#8000FF",
  "#400000", "#804000", "#004000", "#004040", "#000080", "#000040", "#400040", "#400080",
  "#FFFFFF", "#808000", "#808040", "#808080", "#408080", "#C0C0C0", "#400040",
]
            ...
            <el-popover
              ...
              <template #reference>
                <div class="tool-item tool-item-color">
                  <svg viewBox="0 0 1024 1024" p-id="6524" width="64" height="64"><path d="M768 800a31.36 31.36 0 0 1-29.44-19.84L512 214.4l-226.56 565.76a32 32 0 1 1-58.88-24.32l256-640a32.64 32.64 0 0 1 58.88 0l256 640a31.36 31.36 0 0 1-17.28 41.6 29.44 29.44 0 0 1-12.16 2.56z" p-id="6525" fill="currentColor"></path><path d="M657.92 544H358.4a32 32 0 0 1 0-64h299.52a32 32 0 1 1 0 64zM800 960h-576a32 32 0 0 1 0-64h576a32 32 0 0 1 0 64z" p-id="6526" fill="currentColor"></path></svg>
                </div>
              </template>
              <div class="editor-color-layout">
                <div class="color-item" v-for="item in colors" :key="item" :style="{'--color': item}" @click="editor.chain().focus().setColor(item).run()"></div>
