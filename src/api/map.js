...
function create_map(area_name = '') {
    ...
    let area_code = tiles_config.code;
    if(!area_code) {
        return;
    }

    let settings = tiles_config.settings;
    let mapCenter = tiles_config.center || [3568, 6286];
    let mapSize = tiles_config.size || [12288, 15360];
    let mapTilesOffset = tiles_config.tilesOffset || [0, 0];
    let extension = tiles_config.extension || 'png';
    ...
    let tiles = create_map_layer(area_code, mapCenter, mapSize, mapTilesOffset, extension);
