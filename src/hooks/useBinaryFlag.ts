 ...
 * 将一个数值转换为二进制标识列表进行处理
 */
export const useBinaryFlag = <T extends Record<string, number>>(flagNumber: Ref<number | undefined>, binaryIndexMap: T) => {
  const flagBoolMap = computed({
    get: () => {
      const binaryBits = (flagNumber.value ?? 0).toString(2).split('').map(Number).reverse()
